...
--- 
# 8. 리액트를 위한 도커 파일 작성
- 개발 환경과 운영 환경의 도커 파일을 다르게 작성
    - dockerfile.dev 처럼
- npm 이 들어있는 베이스 이미지 중 하나가 node

1. `FROM` : 베이스 이미지 선택
2. `WORKDIR` : 컨테이너 안에서 앱을 시작하는데, 컨테이너 내부 어느 경로에 리액트 관련 파일을 넣을 것인지 선택
3. `COPY` : 로컬 파일을 위의 WORKDIR 로 설정한 컨테이너 경로에 복사
4. `RUN` : 명령어 실행 (보통 npm install 을 실행)
5. `COPY` : 전체 디렉토리에 있는 것을 카피하고 컨테이너 안에 복사
6. `CMD` : 실행 명령어 작성